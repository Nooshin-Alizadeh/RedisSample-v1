...
*   **RedisSample.Server.Api**: Houses API controllers, mappers, the `DbContext`, EF Core migrations, email templates, action filters, SignalR hubs, and server-specific configuration.
*   **RedisSample.Server.Web**: The application's default startup project and entry point. It hosts `App.razor` and configures Blazor Server and server-side rendering (SSR).
*   **RedisSample.Server.Shared**: (Also known as Aspire's ServiceDefaults) Contains common code shared between the `RedisSample.Server.Api` and `RedisSample.Server.Web` projects.
*   **RedisSample.Shared**: Contains shared DTOs, enums, custom exceptions, shared services, and `.resx` resource files.
*   **RedisSample.Tests**: Contains all UI and integration tests.
*   **RedisSample.Client.Core**: The heart of the client application. Contains all shared Blazor components, pages, layouts, client-side services, and the primary `App.ts` and `App.scss` files.
*   **RedisSample.Client.Web**: The Blazor WebAssembly (WASM) standalone project.
*   **RedisSample.Client.Maui**: The .NET MAUI Blazor Hybrid project for native mobile and desktop apps.
*   **RedisSample.Client.Windows**: The Windows Forms Blazor Hybrid project.
...
-   **Build the project**: Run `dotnet build` in RedisSample.Server.Web project root directory.
-   **Run the project**: Run `dotnet watch` in RedisSample.Server.Web project root directory. If needed, you may use the Playwright MCP tools to interact with the running UI to validate things (navigate, click, fill forms, take screenshots), and use `browser_evaluate` to run in-page JavaScript to accelerate the process (e.g. quickly locating elements, extracting data, or asserting state).
-   **Run tests**: Run `dotnet test` in RedisSample.Tests project root directory.
-   **Add new migrations**: Run `dotnet ef migrations add <MigrationName> --output-dir Data/Migrations --verbose` in RedisSample.Server.Api project root directory.
-   **Generate Resx C# code**: Run `dotnet build -t:PrepareResources` in RedisSample.Shared project root directory.
