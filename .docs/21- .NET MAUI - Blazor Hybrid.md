...
4. [RedisSample.Client.Windows Advantages](#redissampleclientwindows-advantages) - Better Windows support
...
The `RedisSample.Client.Maui` and `RedisSample.Client.Windows` projects demonstrate this architecture in action.
...
**Location**: [`/src/Client/RedisSample.Client.Maui/Platforms/Android/MainActivity.cs`](/src/Client/RedisSample.Client.Maui/Platforms/Android/MainActivity.cs)
...
#### iOS - Universal Links
For iOS, this is handled automatically by the `apple-app-site-association` file in the `wwwroot` of the `RedisSample.Client.Web` project. No manual configuration is needed per page!

**Location**: [`/src/Client/RedisSample.Client.Web/wwwroot/.well-known/apple-app-site-association`](/src/Client/RedisSample.Client.Web/wwwroot/.well-known/apple-app-site-association)
...
### ApplicationVersion in RedisSample.Client.Maui

The `ApplicationVersion` property in the `RedisSample.Client.Maui.csproj` file manages the app's version for store submissions and updates.
...
```xml
<!-- In RedisSample.Client.Maui.csproj -->
...
## RedisSample.Client.Windows Advantages

The `RedisSample.Client.Windows` project offers several advantages over the MAUI Windows output:

### Key Benefits

1. **Broader OS Support**: Works on Windows 7, 8, 10, and 11
   - In contrast, `RedisSample.Client.Maui`'s Windows output only works on Windows 10 and 11
...
The app checks the WebView version on Android to ensure compatibility in [`App.xaml.cs`](/src/Client/RedisSample.Client.Maui/App.xaml.cs):
...
The MAUI project is located at: `src/Client/RedisSample.Client.Maui/`
...
The [`MauiProgram.cs`](/src/Client/RedisSample.Client.Maui/MauiProgram.cs) file is the starting point of the MAUI application:
...
public static MauiApp CreateMauiApp()
{
    ...
    builder
        ...
        .UseAppStoreInfo()
        .UseSentry(options => { /* ... */ })
        .Configuration.AddClientConfigurations(clientEntryAssemblyName: "RedisSample.Client.Maui");
...
Services specific to MAUI are registered in [`MauiProgram.Services.cs`](/src/Client/RedisSample.Client.Maui/MauiProgram.Services.cs):
...
The [`MainPage.xaml`](/src/Client/RedisSample.Client.Maui/MainPage.xaml) file hosts the `BlazorWebView` component, which renders Blazor content inside a native WebView:
...
- It renders the `Routes` component (from `RedisSample.Client.Core`) inside the `#app-container` element
- All your Blazor components render inside this WebView, just like in the browser!

### 4. BlazorWebView Customization

The `SetupBlazorWebView()` method in [`MauiProgram.cs`](/src/Client/RedisSample.Client.Maui/MauiProgram.cs) customizes the WebView for each platform:
...
Implements `IStorageService` using MAUI's `Preferences` API in [`MauiStorageService.cs`](/src/Client/RedisSample.Client.Maui/Infrastructure/Services/MauiStorageService.cs):
...
Implements `IExternalNavigationService` for opening external URLs in [`MauiExternalNavigationService.cs`](/src/Client/RedisSample.Client.Maui/Infrastructure/Services/MauiExternalNavigationService.cs):
