...
Welcome to Stage 24! In this stage, you'll learn about the advanced WebAuthn implementation in the RedisSample project, which enables secure passwordless authentication using biometric methods like fingerprint, Face ID, and PIN across all platforms.

---

## 🔐 What is WebAuthn?

**WebAuthn** (Web Authentication) is a modern web standard that enables passwordless authentication using public-key cryptography. The RedisSample project implements a comprehensive WebAuthn solution that works across:
...
**3. Server Generates Challenge** (`src/Server/RedisSample.Server.Api/Features/Identity/UserController.WebAuthn.cs`)
...
**4. Platform-Specific Credential Creation**
   ...
   **Web Platform** (`src/Client/RedisSample.Client.Core/Infrastructure/Services/WebAuthnService.cs`):
   - Directly calls browser's WebAuthn API via Bit.Butil
   - Browser shows native biometric prompt
   
   **Blazor Hybrid** (`src/Client/RedisSample.Client.Maui/Infrastructure/Services/MauiWebAuthnService.cs`, `src/Client/RedisSample.Client.Windows/Infrastructure/Services/WindowsWebAuthnService.cs`):
...
**6. Server Validates and Stores** (`src/Server/RedisSample.Server.Api/Features/Identity/UserController.WebAuthn.cs`)
...
**3. Server Generates Assertion Options** (`src/Server/RedisSample.Server.Api/Features/Identity/IdentityController.WebAuthn.cs`)
...
**6. Server Verifies Assertion** (`src/Server/RedisSample.Server.Api/Features/Identity/IdentityController.WebAuthn.cs`)
   - Retrieves cached options using challenge
...
**`WebAuthnCredential` Entity** (`src/Server/RedisSample.Server.Api/Features/Identity/Models/WebAuthnCredential.cs`):
...
**`WebAuthnServiceBase`** (`src/Client/RedisSample.Client.Core/Infrastructure/Services/WebAuthnServiceBase.cs`):
...
**`ILocalHttpServer`** Interface (`src/Client/RedisSample.Client.Core/Infrastructure/Services/Contracts/ILocalHttpServer.cs`):
...
**Location**: `src/Client/RedisSample.Client.Web/wwwroot/web-interop-app.html`
...
**Key Script** (`src/Client/RedisSample.Client.Core/Scripts/WebInteropApp.ts`):
...
**`UserController.WebAuthn.cs`** (`src/Server/RedisSample.Server.Api/Features/Identity/UserController.WebAuthn.cs`) - Credential Management:
...
**`IdentityController.WebAuthn.cs`** (`src/Server/RedisSample.Server.Api/Features/Identity/IdentityController.WebAuthn.cs`) - Authentication:
...
{
    options.ServerName = "RedisSample WebAuthn";
