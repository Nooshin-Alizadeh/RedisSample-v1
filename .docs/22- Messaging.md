...
Welcome to Stage 22! In this stage, you will learn about the comprehensive **messaging and real-time communication system** built into the RedisSample project. This system provides a unified messaging architecture that enables communication across multiple channels and platforms.
...
At the heart of the RedisSample messaging architecture is **AppMessages** - a centralized messaging system that provides a consistent way to communicate across different parts of your application, regardless of whether the communication happens:
...
**Location**: [`src/Client/RedisSample.Client.Core/Infrastructure/Services/ClientAppMessages.cs`](/src/Client/RedisSample.Client.Core/Infrastructure/Services/ClientAppMessages.cs)
...
The RedisSample project provides multiple communication channels that all work with the same centralized message system. Let's explore each channel and understand when to use them.
...
**Location**: [`src/Client/RedisSample.Client.Core/Infrastructure/Services/PubSubService.cs`](/src/Client/RedisSample.Client.Core/Infrastructure/Services/PubSubService.cs)
...
**SignalR** enables the server to send messages to clients in real-time. In the RedisSample project, SignalR messages are automatically bridged to PubSubService, creating a seamless experience.

**Server-Side Hub**: [`src/Server/RedisSample.Server.Api/Infrastructure/SignalR/AppHub.cs`](/src/Server/RedisSample.Server.Api/Infrastructure/SignalR/AppHub.cs)
...
**Location**: [`src/Client/RedisSample.Client.Core/Components/Layout/AppJsBridge.razor.cs`](/src/Client/RedisSample.Client.Core/Components/Layout/AppJsBridge.razor.cs)
...
The `window.postMessage` API allows communication between different JavaScript contexts (e.g., iframes, service workers). The RedisSample project bridges this to PubSubService.

**Location**: [`src/Client/RedisSample.Client.Core/Scripts/events.ts`](/src/Client/RedisSample.Client.Core/Scripts/events.ts)
...
**Location**: [`src/Client/RedisSample.Client.Web/wwwroot/service-worker.js`](/src/Client/RedisSample.Client.Web/wwwroot/service-worker.js)
...
**Location**: [`src/Client/RedisSample.Client.Core/Components/AppClientCoordinator.cs`](/src/Client/RedisSample.Client.Core/Components/AppClientCoordinator.cs)
...
The RedisSample project supports **push notifications** across all platforms:
...
**Client-Side Interface**: [`src/Client/RedisSample.Client.Core/Services/Contracts/IPushNotificationService.cs`](/src/Client/RedisSample.Client.Core/Services/Contracts/IPushNotificationService.cs)

**Base Implementation**: [`src/Client/RedisSample.Client.Core/Services/PushNotificationServiceBase.cs`](/src/Client/RedisSample.Client.Core/Services/PushNotificationServiceBase.cs)
...
**Location**: [`src/Server/RedisSample.Server.Api/Services/PushNotificationService.cs`](/src/Server/RedisSample.Server.Api/Services/PushNotificationService.cs)
...
**Extension Helper**: [`src/Client/RedisSample.Client.Core/Extensions/NotificationExtensions.cs`](/src/Client/RedisSample.Client.Core/Extensions/NotificationExtensions.cs)
...
When testing push notifications, it's critical to understand that there are **four distinct scenarios** based on the app state when the notification is sent and when the user taps on it. The RedisSample project handles all four scenarios across all platforms.
...
* [Describe the workflow of bit RedisSample's AI chat feature and provide a high-level overview.
