...
This stage explains how Dependency Injection (DI) is structured in the RedisSample project and where services are registered for different platforms and environments.
...
2. **`IClientCoreServiceCollectionExtensions.cs`** ([`src/Client/RedisSample.Client.Core/Extensions/`](/src/Client/RedisSample.Client.Core/Extensions/IClientCoreServiceCollectionExtensions.cs))
...
3. **`Program.Services.cs`** files in each project
   ...
   | File | Purpose |
   |------|---------|
   | `RedisSample.Server.Api/Program.Services.cs` | API server services (DbContext, Identity, Email, SMS, Push Notifications, AI, Hangfire) |
   | `RedisSample.Server.Web/Program.Services.cs` | Blazor Server/SSR services (combines API + Client services) |
   | `RedisSample.Client.Web/Program.Services.cs` | Blazor WebAssembly-specific services |
   | `RedisSample.Client.Maui/MauiProgram.Services.cs` | MAUI services (Android, iOS, macOS, Windows via MAUI) |
   | `RedisSample.Client.Windows/Program.Services.cs` | Windows Forms Blazor Hybrid services |
...
4. **Platform-specific extensions** (MAUI only)
   ...
   |---------------|----------|
   | [`IAndroidServiceCollectionExtensions.cs`](/src/Client/RedisSample.Client.Maui/Platforms/Android/Extensions/IAndroidServiceCollectionExtensions.cs) | Android-specific services (e.g., `AndroidPushNotificationService`) |
   | [`IIosServiceCollectionExtensions.cs`](/src/Client/RedisSample.Client.Maui/Platforms/iOS/Extensions/IIosServiceCollectionExtensions.cs) | iOS-specific services |
   | [`IMacServiceCollectionExtensions.cs`](/src/Client/RedisSample.Client.Maui/Platforms/MacCatalyst/Extensions/IMacServiceCollectionExtensions.cs) | macOS-specific services |
   | [`IWindowsServiceCollectionExtensions.cs`](/src/Client/RedisSample.Client.Maui/Platforms/Windows/Extensions/IWindowsServiceCollectionExtensions.cs) | Windows (MAUI)-specific services |
...
Located in [`IClientCoreServiceCollectionExtensions.cs`](/src/Client/RedisSample.Client.Core/Extensions/IClientCoreServiceCollectionExtensions.cs):
...
1. **Define the interface** in `RedisSample.Shared` or `RedisSample.Client.Core`
...
```csharp
// src/Client/RedisSample.Client.Core/Services/FeedbackService.cs
namespace RedisSample.Client.Core.Services;
...
```csharp
// src/Client/RedisSample.Client.Core/Extensions/IClientCoreServiceCollectionExtensions.cs
