...
Welcome to Stage 20! In this stage, you'll learn about .NET Aspire and how it transforms both development and deployment experiences in this RedisSample project.
...
**Location**: [`src/Server/RedisSample.Server.AppHost/`](/src/Server/RedisSample.Server.AppHost/)
...
// Main server project
var serverWebProject = builder.AddProject("serverweb", "../RedisSample.Server.Web/RedisSample.Server.Web.csproj")
...
if (builder.ExecutionContext.IsRunMode)
{
    ...
    builder.AddProject("clientwebwasm", "../../Client/RedisSample.Client.Web/RedisSample.Client.Web.csproj")
        .WithExplicitStart();
    ...
    builder.AddProject("clientwindows", "../../Client/RedisSample.Client.Windows/RedisSample.Client.Windows.csproj")
        .WithExplicitStart();
...
1. Open `Program.cs` in the `RedisSample.Server.AppHost` project
...
**Location**: [`src/Server/RedisSample.Server.AppHost/appsettings.Development.json`](/src/Server/RedisSample.Server.AppHost/appsettings.Development.json)
...
- ❌ `RedisSample.Server.Api` **does NOT** run on Docker
- ❌ `RedisSample.Server.Web` **does NOT** run on Docker
...
3. Run this command in the terminal to clean previous Windows build artifacts:
   ...
   ```
4. Then run the `RedisSample.Server.AppHost` project
