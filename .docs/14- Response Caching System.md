...
The `AppResponseCachePolicy` class (located in `/src/Server/RedisSample.Server.Shared/Infrastructure/Services/AppResponseCachePolicy.cs`) implements the actual caching logic. It's an implementation of ASP.NET Core's `IOutputCachePolicy` interface.
...
The `ResponseCacheService` (located in `/src/Server/RedisSample.Server.Api/Infrastructure/Services/ResponseCacheService.cs`) provides methods to **purge/invalidate cached responses** when data changes.
...
The `CacheDelegatingHandler` (located in `/src/Client/RedisSample.Client.Core/Infrastructure/Services/HttpMessageHandlers/CacheDelegatingHandler.cs`) implements client-side in-memory caching for HTTP responses.
...
* [How does the bit RedisSample AttachmentController interact with response caching? Why do users always see the latest profile pictures, even though no PurgeCache has been called and these assets are stored in the browser cache, which cannot be automatically purged?](https://deepwiki.com/search/how-does-the-bit-redissample-a_4f042d5f-3ffb-4c14-b661-bb923825c21d)
* [Why response caching doesn't work with stream pre-rendering in bit RedisSample?](https://deepwiki.com/search/why-response-caching-doesnt-wo_2de1ba6c-1017-4c77-96f5-33c8ed001760)
