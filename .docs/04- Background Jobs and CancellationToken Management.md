...
Welcome to **Stage 4** of the RedisSample project tutorial! In this stage, we'll explore how the project handles **cancellation tokens** for request cancellation and **background job processing** with Hangfire.
...
In this stage, we will explore how this redissample harmonizes these three pillars to keep your application efficient, safe, and reliable.
...
Let's look at a real controller from the project - [`TodoItemController.cs`](/src/Server/RedisSample.Server.Api/Features/Todo/TodoItemController.cs):
...
Let's examine how `CurrentCancellationToken` is implemented in [`AppComponentBase.cs`](/src/Client/RedisSample.Client.Core/Components/AppComponentBase.cs):
...
Here's a real example from [`TodoPage.razor.cs`](/src/Client/RedisSample.Client.Core/Components/Pages/TodoPage.razor.cs):
...
In [`PhoneService.cs`](/src/Server/RedisSample.Server.Api/Infrastructure/Services/PhoneService.cs):
...
In [`PhoneServiceJobsRunner.cs`](/src/Server/RedisSample.Server.Api/Infrastructure/Services/PhoneServiceJobsRunner.cs):
...
In [`Program.Services.cs`](/src/Server/RedisSample.Server.Api/Program.Services.cs):
