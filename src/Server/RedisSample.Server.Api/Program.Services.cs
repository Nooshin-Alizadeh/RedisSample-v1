...
using RedisSample.Server.Api.Features.Statistics;
using RedisSample.Shared.Infrastructure.Resources;
using RedisSample.Server.Api.Infrastructure.RequestPipeline;
...
public static partial class Program
{
    public static void AddServerApiProjectServices(this WebApplicationBuilder builder)
    {
        ...
        {
            return new Medallion.Threading.FileSystem.FileDistributedLock(new(Path.Combine(Path.GetTempPath(), $"RedisSample-{lockKey}.lock")));
        }));
        ...
        services.AddScoped(sp =>
        {
            ...
            var options = new Fido2Configuration
            {
                ...
                ServerName = "RedisSample WebAuthn",
        ...
        services.AddHangfire((sp, hangfireConfiguration) =>
        {
            ...
            else
            {
                ...
                {
                    optionsBuilder.UseSqlite($"Data Source={Path.Combine(appDataDirPath, "RedisSampleJobDb.db")};");
                }, new()
                {
                    Schema = "jobs",
                    QueuePollInterval = new TimeSpan(0, 0, 1)
