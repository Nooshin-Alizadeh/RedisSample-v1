...
namespace RedisSample.Server.Api.Features.Identity.Services;
    ...
    : UserClaimsPrincipalFactory<User, Role>(userManager, roleManager, optionsAccessor)
{
    ...
    private async Task<string> GetKeycloakAccessToken(User user, string? keycloakRefreshToken, string realm)
    {
        ...
        await using var distributedLock = await distributedLockProvider($"RedisSample-Locks-KeycloakTokenRefresh-{user.Id}").AcquireAsync(TimeSpan.FromSeconds(10));
