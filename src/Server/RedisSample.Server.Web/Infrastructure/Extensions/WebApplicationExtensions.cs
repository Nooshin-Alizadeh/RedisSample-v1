...
<sitemapindex xmlns=""http://www.sitemaps.org/schemas/sitemap/0.9"">
   <sitemap>
      <loc>{0}sitemap.xml</loc>
            ...
            {
                var urls = AssemblyLoadContext.Default.Assemblies.Where(asm => asm.GetName().Name?.Contains("RedisSample.Client") is true)
    ...
    {string.Join(Environment.NewLine, urls.Select(u => $"<url><loc>{new Uri(baseUrl, u)}</loc></url>"))}
            ...
            app.MapGet("/llms.txt", [AppResponseCache(SharedMaxAge = 3600 * 24 * 7)] async (context) =>
            {
                ...
                var llms = @$"# RedisSample

> RedisSample is a cross-platform application available on Android, iOS, Windows, macOS and as a Web (PWA) app.
