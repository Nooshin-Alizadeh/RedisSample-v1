...
public static partial class IConfigurationBuilderExtensions
{
    extension(IConfigurationBuilder builder)
    {
        ...
        public IConfigurationBuilder AddClientConfigurations(string clientEntryAssemblyName)
        {
            ...
            var sharedAssembly = Assembly.Load("RedisSample.Shared");

            configBuilder.AddJsonStream(sharedAssembly.GetManifestResourceStream("RedisSample.Shared.appsettings.json")!);

            var envSharedAppSettings = sharedAssembly.GetManifestResourceStream($"RedisSample.Shared.appsettings.{AppEnvironment.Current}.json");
            if (envSharedAppSettings != null)
            {
                configBuilder.AddJsonStream(envSharedAppSettings);
            }

            var clientCoreAssembly = AppDomain.CurrentDomain.GetAssemblies().Single(asm => asm.GetName()?.Name is "RedisSample.Client.Core");

            configBuilder.AddJsonStream(clientCoreAssembly.GetManifestResourceStream("RedisSample.Client.Core.appsettings.json")!);

            var envClientCoreAppSettings = clientCoreAssembly.GetManifestResourceStream($"RedisSample.Client.Core.appsettings.{AppEnvironment.Current}.json");
